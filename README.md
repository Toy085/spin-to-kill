# Spin To Kill

A game made in Godot. This game was origanally made for [**The Very Serious Juniper Dev Game Jam**](https://itch.io/jam/theveryseriousjuniperdevgamejam).

### Development

1. Clone the repo
```sh
git clone https://github.com/Toy085/spin-to-kill
```

2. Open the project.godot file in godot

## How to Play
1. Download the game from [Itch.io](https://toy085.itch.io/stk).
2. Extract (unzip) the zip file on your computer.
3. Open the folder and run `spin to kill.exe`.

### Controls

* **Move:** WASD / Left stick
* **Attack Left:** Q / Left Click / Left shoulder button
* **Attack Right:** E / Right Click / Right shoulder button
* **Aim Axe Throw:** Mouse / Right stick
* **Throw Axe (Keyboard + Mouse only):** Space
* **Pause:** Escape 


## Screenshots
<img width="1861" height="1049" alt="image" src="https://github.com/user-attachments/assets/5ccf8651-0cc5-480e-a195-bcbbf493ba52" />

## AI use
AI has been used for making the shop item cards scripts and shop roulette. AI was also used for some code completion.
